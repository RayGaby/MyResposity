...
public class UserInfoDao implements I_UserInfoDao {
	...
	public int insertUserInfo(HashMap map) {
		...
		String photo_id = (String) map.get("photo_id");
	...
	public int updateUserInfo(HashMap map) {
		...
		Long organization_id = Db.queryLong(sql1.toString(), organization);
		System.out.println(organization_id);
		StringBuffer sql2 = new StringBuffer();
		sql2.append("SELECT	id	FROM	role	WHERE	name = ? ");
		Long role_id = Db.queryLong(sql2.toString(), role);
	...
	public List<Record> searchExsitingdata(String id) {
		...
		sql.append("WHERE ");
		sql.append("	u.id=? ");
		List<Record> exsitingdata = Db.find(sql.toString(), id);
		return exsitingdata;
	}
	...
	public int updateUserBlock(String id, int block) {
		StringBuffer sql = new StringBuffer();
		sql.append("UPDATE `user` SET block=? WHERE id=?; ");
	...
	public String searchPhotoId(String id) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT photo_id FROM `user` WHERE id=?; ");
		String photo_id = Db.queryStr(sql.toString(), id);
		return photo_id;
	}
	...
	public boolean userNameIfExist(String userName) {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT ");
		sql.append("	count(DISTINCT u.username) ");
		sql.append("FROM ");
		sql.append("	`user` u ");
		sql.append("WHERE ");
		sql.append("	u.username = ? ");
